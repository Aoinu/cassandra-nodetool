...
const { println, printJSON } = require('./print');

const humanReadableFormatter = (ringStatus) => {
...
exports.humanReadableFormatter = humanReadableFormatter;

exports.ringStatusReporter = async (args) => {
    const ringStatus = await Nodetool(args).status(args);
    if (args.json) {
        printJSON(ringStatus, args.space);
    } else {
        humanReadableFormatter(ringStatus);
    }
};